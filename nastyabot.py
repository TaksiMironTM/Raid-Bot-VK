...
vk_session = vk_api.VkApi(token='ТОКЕН') #Замените ТОКЕН на свой token от группы 
vk = vk_session.get_api()
longpoll = VkBotLongPoll(vk_session,123) #Замените 123 на свой ID группы 
def main():

	keyboard = VkKeyboard(one_time=False) 
	keyboard.add_button("NASTYA Shturm TaksiMironTM vto.pe", color=VkKeyboardColor.DEFAULT)
	keyboard.add_line()	
	keyboard.add_button("NASTYA Shturm TaksiMironTM vto.pe", color=VkKeyboardColor.DEFAULT)
	keyboard.add_line()
